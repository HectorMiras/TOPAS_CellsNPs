...
// Helper: create an orb with pre-check and debug
static G4Orb* NewG4OrbChecked(const G4String& name, G4double radius) {
    auto tol = 10.0 * G4GeometryTolerance::GetInstance()->GetRadialTolerance();
    G4cout << "[TsNucleus][CHECK] Request G4Orb \"" << name
           << "\" radius=" << radius/nm << " nm (threshold="
           << tol/nm << " nm)" << G4endl;
    if (radius < tol) {
        G4cerr << "[TsNucleus][ERROR] G4Orb \"" << name << "\" radius="
               << radius/nm << " nm < 10*kCarTolerance=" << tol/nm << " nm" << G4endl;
        G4Exception("TsNucleus::Construct","GeomSolids0002-Pre",FatalException,
                    ("Invalid radius for subcomponent \"" + name + "\"").c_str());
    }
    if (radius < 2.0*tol) {
        G4cout << "[TsNucleus][WARN] G4Orb \"" << name << "\" radius near tolerance: "
               << radius/nm << " nm (threshold " << tol/nm << " nm)" << G4endl;
    }
...
G4VPhysicalVolume* TsNucleus::Construct()
{
	...
	// Nucleus Orb
	G4Orb * Nucleus_solid = NewG4OrbChecked("Nucleus", fNucleusRadius);
							...
							G4bool BuildSphere)
{
	...
	if (BuildSphere){
		if (fAddBases){
		 ...
		 if (fAddBackbones){
			 G4Orb* gDNA_backbone = NewG4OrbChecked("DNA_backbone", 0.271*nm);
